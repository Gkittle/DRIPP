...
######## initialize vectors for simulation
    ...
    ############# extract action from policy 
            ...
            if any( [policy_con=='d5', policy_con=='d10', policy_con=='d15', policy_con=='d20', policy_con=='d25'] ):
            ...
            deficit = max( 0, self.demand[(t%12)]*(1 - reduction_amount[t]/100) - max(0,rswp[t+1]) - max(0, rc[t+1]) - max(0, rgi[t+1]) - max(0, md[t]) - installed_capacity[t]) #Changed from original to not penalize curtailed demand
            if deficit < 1e-10:
                deficit = 0
